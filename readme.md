...
This is an example of a brittle test written againt [MvcMusicStore]() website:
...
Host is a static class with a single static property Instance which upon instantiation fires up IIS Express on the website under test and binds Selenium WebDriver to the browser instance.

You can find the complete code [here]().

## Magic Ingredient
So UI tests are hard to write and maintain but you can follow a few rules and patterns to have a much more maintainable UI test suite!

There are patterns that allow you to write more maintainable UI test suite and many of these patterns have been out there for quite some time but they are not promoted as much as needed. In this post I will try to share my learnings and how I write UI tests nowadays and hopefully help ease this pain for other developers and testers who are struggling with UI testing. 

These patterns are platform agnostic. I have used these very same ideas/patterns/tips to write UI tests for WPF applications and web applications written in ASP.Net and Ruby on Rails. So regardless of your technology stack you should be able to make your UI tests a lot more maintainable by applying these rules. 

One last thing: you can find the code for this article on the [Seleno repository](https://github.com/TestStack/TestStack.Seleno). The sample I am going to be using is MvcMusicStore which you can find [here](https://github.com/TestStack/TestStack.Seleno/tree/master/src/Samples/MusicStore).

## Test Code Is Code!
You either write tests or you don't. If you do, then much like your actual code, you are going have to maintain your tests. So give them the same treatment. What is it about tests that makes us think we can forego quality in them? If anything a bad test suite is going to take a lot longer to maintain than bad code. I have had bad peices of working code in production for years which never broke and I never had to touch them. Sure it was ugly as hell but it worked and it didn't need change and never broke. The situation is not quite the same for bad tests though: because bad tests are going to break and fixing them is going to be hard. 
...
So if you have to maintain your tests the same way you maintain your code, then you should give it some love. Test code is code. Do you apply [SRP](http://www.objectmentor.com/resources/articles/srp.pdf) on your code? Then you should apply it on your tests too. Is your code [DRY](http://c2.com/cgi/wiki?DontRepeatYourself)? Then DRY up your tests too. If you don't write good tests (UI or otherwise) you will waste a lot of time maintaining them. I cannot even count the number of times I have seen developers avoid testing because they think writing tests is a huge waste of time because it takes too much time to maintain. 

## From horrid to awesome in four steps
Let's start with the failed experience. As mentioned the ideas and patterns I am going to discuss are agnostic of the technology stack and you don't need a framework to apply them. For this article though I am going to use Selenium in .Net. So "I" have this online music store called [MVC MusicStore](http://mvcmusicstore.codeplex.com/) and I am going to write some UI automation for it. 

Again, you can find the entire code for this article [here](). 

### Horrid
I start with screen recoding using Selenium IDE and I end up with something like [this](https://github.com/TestStack/TestStack.Seleno/blob/master/src/Samples/MusicStore/MvcMusicStore.FunctionalTests/BrittleTests.cs):
...
Actually the script Selenium IDE generated for me was a lot worse and I had to clean up the bit on the top where it fires up the browser. This way at least at I can assure that my browser instantiation code is not duplicated all over the place!!

So what is wrong with that code? 

 - This is procedural code. When reading the code, if you can actually read it 10 minutes after you generate it, you cannot really draw a line between different pages. It's all a bit pile of code where we get the 'driver' to find an element on the page and to do something with it!
 - It results to duplicate logic. For example the bit on the top is registering a new user. If you had to register in two different tests going down this path would duplicate that logic. For something like login page which could be a prerequisite in a lot of your tests that means duplicating the same bit of code in most of your tests which is why when Username is replaced with Email address most of your tests break. 
 - It results to duplicated selectors. Selector is how to target an element on your view from your test script. For my users I currently have FirstName and LastName; but then the business asks me to change that to Forename and Surname (this very same thing happened to me in a project about a year ago)! This simple change means too many broken tests because I have used FirstName and LastName everywhere in my tests.
 - There is a lot of magic strings everywhere. Even if I had used FirstName and LastName in all my tests but instead of using magic strings I had somehow extracted that out to change the FirstName to Forename and LastName to Surname I would really have to change where these "fields" were defined instead of everywhere in my code and then refactoring tools would allow me to simply rename the references.
 
So let's fix these issues.

### Step 1: DRY your tests
...
I have gone ahead and refactored the logics and interactions out of the single brittle test into a few page objects. You can find the new test [here](https://github.com/TestStack/TestStack.Seleno/blob/master/src/Samples/MusicStore/MvcMusicStore.FunctionalTests/Step1/PageObject.cs). The code is copied here for your reference:
    ...
    public void Can_buy_an_Album_when_registered()
    {
        ...
        // PlaceOrder method not shown for brevity
        var orderedPage = PlaceOrder(homepage);
        orderedPage.Title.Should().Be("Checkout Complete");
    }

For a second here please ignore the Seleno API and let's talk about Page Object and what it did for us. When you implement Page Objects, a textbox in your view becomes a string property on the Page Object and to fill that textbox your just set that text property to the desired value. So instead of
...
when registerPage is an instance of the [RegisterPage](https://github.com/TestStack/TestStack.Seleno/blob/master/src/Samples/MusicStore/MvcMusicStore.FunctionalTests/Step1/Pages/RegisterPage.cs). Likewise a link on the view becomes a method on the Page Object and clicking the link turns into calling the method on the Page Object. So instead of
...
In fact any action on our view becomes a method in our page object and the beauty of this model is that in response to taking an action on a view (or page object) you get an instance of another view (or I should say page object). So you can easily chain your view interaction. Here is a bit of code from the `PlaceOrder` method (as removed from the above snippet):
...
On an instance of the [HomePage](https://github.com/TestStack/TestStack.Seleno/blob/master/src/Samples/MusicStore/MvcMusicStore.FunctionalTests/Step1/Pages/HomePage.cs) class I call `SelectGenreByName` which clicks on a 'Disco' link on the page which returns an instance of [AlbumBrowsePage](https://github.com/TestStack/TestStack.Seleno/blob/master/src/Samples/MusicStore/MvcMusicStore.FunctionalTests/Step1/Pages/AlbumBrowsePage.cs) and then on that page I call `SelectAlbumByName` which clicks on the 'Le Freak' album and returns an instance of [AlbumDetailsPage](https://github.com/TestStack/TestStack.Seleno/blob/master/src/Samples/MusicStore/MvcMusicStore.FunctionalTests/Step1/Pages/AlbumDetailPage.cs) and on that page I click the `AddToCart` button which adds the CD to my shopping cart and takes me to the shopping cart where I can `Checkout`. I admit it it is a lot of classes for what used to be no class at all; but we gained a lot of benefits from this practice. Firstly the code is no longer procedural. We have a well contained testing model where each object provides nice encapsulation of interaction with a page. So for example if something changes in your registration logic the only place you have to change is your RegisterPage class instead of having to go through your entire test suite and change every single interaction with the registration view. This modularity also provides for nice reuse; for example you can reuse your Shopping Cart page everywhere you need to interact with the shopping cart. So in a simple practice of moving from procedural to object oriented test code we **almost** eliminated three of the four issues with the initial brittle test which were procedural code, and logic and selector duplication. We still have a little bit of duplication though which we will fix shortly.

Before we go any further let's take a quick look under the bonnet. How did we actually implemented those page objects. A page object in it's root is nothing but a wrapper around the normal interaction you have with the page. So in your implementation you could just extract UI interactions our of the brittle tests and put them into their own page objects. For example you could write a RegisterPage that looked like this:
